...
async function demoStream(notes, send) {
  ...
  const isQuestion = (l) => /(\?|？|要検討|未定|どうする|保留)/.test(l);
  const owner = (l) => (l.match(/([一-龥ぁ-んァ-ンA-Za-z]{2,6})(さん|くん|氏)/)?.[1] ?? "未定");
  const due = (l) => (l.match(/(\d{1,2}\/\d{1,2}|来週|今週|明日|月末|[0-9]+日まで)/)?.[1] ?? "未定");

  for (const t of ["メモを読み込み中…", "決定事項とタスクを分類しています…", "Issue 下書きを組み立てています…"]) {
    send("thinking", { text: `[デモ] ${t}\n` });
    await sleep(350);
  }
  const action_items = lines.filter(isTask).map((l, i) => ({ task: l, owner: owner(l), due: due(l), priority: i === 0 ? "high" : "medium" }));
  const result = {
    title: lines[0]?.slice(0, 30) || "会議",
    summary: `（デモモード：API キー未設定のため Fable 5.1 は呼んでいません）${lines.length} 行のメモから、決定事項 ${lines.filter(isDecision).length} 件・タスク ${action_items.length} 件を抽出しました。`,
    decisions: lines.filter(isDecision),
    action_items,
    open_questions: lines.filter(isQuestion),
